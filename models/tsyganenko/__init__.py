...
from hatch_python_utils.models.tsyganenko import tsygFort
import logging
...
class tsygTrace(object):
    ...
    **FUNCTION**: trace(lat, lon, rho, coords='geo', datetime=None,
    ...
    def __init__(self, lat=None, lon=None, rho=None, filename=None,
        ...
        haveStuff = all(elem is not None for elem in [lat, lon, rho])
        assert haveStuff or filename, 'You must provide either (lat, lon, rho) or a filename to read from'
    ...
    def __test_valid__(self):
        ...
        assert (self.coords.lower() == 'geo'), '{}: this coordinae system is not supported'.format(
            self.coords.lower())
    ...
    def trace(self, lat=None, lon=None, rho=None, coords=None, datetime=None,
        ...
        self.l = zeros(len(lat), dtype=np.int64)
...
({:6.3f}, {:6.3f}, {:6.3f}) @ {}
    ...
    def plot(self, proj='xz', color='b', onlyPts=None, showPts=False,
        ...
        **kwargs :
        ...
        if showEarth:
            circ = Circle(xy=(0, 0), radius=1, facecolor='0.8',
                          edgecolor='k', alpha=.5, zorder=0)
            ax.add_patch(circ)
    ...
    def plot3d(self, onlyPts=None, showEarth=True, showPts=False, disp=True,
        ...
        **kwargs :
        ...
        if showEarth:
            ...
            ax.plot_surface(tx, ty, tz, rstride=10, cstride=10,
                            color='grey', alpha=.5, zorder=0, linewidth=0.5)
        ...
        for ip in inds:
            ...
            if showPts:
                ax.scatter3D(self.xGsw[ip], self.yGsw[ip],
                             self.zGsw[ip], c='k')
