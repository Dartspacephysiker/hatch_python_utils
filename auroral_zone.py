...
def auroral_zone(mlt, q,
    ...
    # n =	  0    1    2     3    4    5    6
    apole = np.array([[15.22, 2.41, 3.34, -0.85, 1.01, 0.32, 0.90],
    ...
    aequa = np.array([[17.36, 3.03, 3.46, 0.42, 2.11, -0.25, 1.13],
    ...
    Andeg = np.array([0, 1, 3, 5])  # The An that are in degrees
    ...
    if keyword_set(pole):
        t = apole(0, i) + \
            apole(1, i)*np.cos(p+apole(2, i)) + \
            apole(3, i)*np.cos(2*(p+apole(4, i))) + \
            apole(5, i)*np.cos(3*(p+apole(6, i)))
    else:
        t = aequa(0, i) + \
            aequa(1, i)*np.cos(p+aequa(2, i)) + \
            aequa(3, i)*np.cos(2*(p+aequa(4, i))) + \
            aequa(5, i)*np.cos(3*(p+aequa(6, i)))
